...
def send_tcp_packet(host, port, points):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            print(f"TCP Packet sent ({len(payload)} bytes): {points} to {host}:{port}")
    except ConnectionRefusedError:
        print(f"Connection refused at {host}:{port}. Make sure the server is running.")
...
if __name__ == "__main__":
    ...
    while True:
        for points in trajectory_data:
            send_tcp_packet(host, port, points)
            time.sleep(2)